...
- [ ] Unaware 进攻／防守矩阵：忽略对方 Stage、两分支置 0、与会心 clamp 组合正确；两侧叠加有覆盖。
- [ ] Infiltrator 仅进攻方绕过 Reflect／Light Screen；防守方恒 `inactive`；错类别／破墙／`+3` 时无增量。
- [ ] Stage／Screen Track 选项不被修改；绕过后与 `0`／`none` 合并并保留 `inactive` provenance。
- [ ] 被绕过 Stage／Screen 不进主行；无绿点／专用 bypass UI；二者无红色 unsupported 提示。
- [ ] 父 issue checklist 可勾选 Unaware 与 Infiltrator。
- [ ] 对 [[../docs/traces/discussion/2026-08-07-ability-stage-and-screen-bypass|讨论记录]] 的每项决定逐行完成实现审计。
