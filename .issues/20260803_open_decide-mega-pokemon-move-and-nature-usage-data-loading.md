...
- 上游数据是否直接提供 Mega Battle Pokémon Identity 的招式与性格分布；若只提供基础 Pokémon Form、队伍或 Mega Stone 记录，如何建立可审计的关联。
- 招式使用率如何关联到 Move Template identity；性格使用率如何关联到受支持的 nature value。
- 数据在资源生成时 vendoring、构建时生成还是运行时加载。
- 缓存、刷新、版本固定、缺失记录和加载失败如何处理。
- 加载结果如何向 Move 默认候选与性格／Stat Preset 逻辑暴露稳定的 usage shape。
