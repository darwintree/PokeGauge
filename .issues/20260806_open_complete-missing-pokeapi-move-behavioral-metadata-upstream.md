...
PokeGauge 以 PokeAPI 作为 Move 资源与行为语义的 source of truth，但 PokeAPI 当前不足以完整表达部分特性所需的 Move 行为：
...
- REST Move resource 不暴露数据库内部的 Move attributes；PokeGauge 当前可直接消费官方 CSV，因此 REST 暴露不是本地实现的前置。

缺行无法区分“明确为 false”与“尚未录入”，直接按 false 消费会把已知正例误判为 `inactive`。

## Goal

向 PokeAPI 上游提交一个或多个 PR，使 PokeGauge 所需的 Move 行为在 PokeAPI 中有完整、结构化、可按 numeric Move ID 消费的记录；上游合并后更新仓库中的 PokeAPI pin。

## Source-of-truth contract

- PokeAPI 是 PokeGauge 运行时与生成时 Move 语义的唯一 source of truth。
- 不为这些缺口新增本地 fallback、Showdown / calc 运行时依赖、按名称匹配或效果文本解析。
- Showdown 等资料可作为上游 PR 的核对证据，但不成为 PokeGauge 的生产数据源。
...
- [ ] 只在 PokeGauge 需要或上游要求时处理 REST Move attribute 暴露；当前 CSV 消费路径足够。
...
- [ ] PokeGauge 生成器已直接消费新的 PokeAPI 结构数据并有回归检查。
