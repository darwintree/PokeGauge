...
- 已确认 kickoff：天气只覆盖直接威力／伤害与命中；双方特性默认全选合法特性（含隐藏特性），本轮只有适应力产生已支持效果；墙只有无墙、反射壁、光墙。
...
- 多个不同 Track 同时产生局部等价时，来源信息在窄屏与高密度结果中的最终压缩形态；先由合并原型提高讨论保真度，再决定是否需要追加问题。
- Move snapshot 编辑原型与合并来源原型可能暴露的共享交互模式，目前尚不足以精确切票。
- 天气研究可能暴露需要单独规格化的例外招式；在支持矩阵形成前不预先猜测其数量与边界。
...
- 代码实现与实现 tickets；等本 map 到达 destination 后再从最终规格拆分。
