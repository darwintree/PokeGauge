...
import { nameUsageRules, selectRecommendedRules } from "./usage-rules"
const rules = (ids: string[]) => ids.map(id => ({ id, label: id }))

it("follows upstream default and numeric season recency", () => {
  expect(selectRecommendedRules("champions", { defaultId: "Live", rules: rules(["M9", "M11", "M10", "Live"]) })).toEqual(["Live", "M11", "M10"])
})
it("chooses latest month/regulation and both ladder extremes without a fixed rating", () => {
  ...
  expect(selectRecommendedRules("smogon", { defaultId: "unused", rules: rules([
...
it("covers current ladder, tournaments, ranked season and historical tournaments", () => {
  ...
  expect(selectRecommendedRules("pikalytics", { defaultId, rules: rules([
