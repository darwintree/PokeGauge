...
export function ResultSetSummary({
  ...
  const dimensions = resultGroupingOptions(trackState).map(({ id, count }) => {
    const shortLabel = ["move", "offense", "item", "defense"].includes(id)
    const dimension = intl.formatMessage({
      id: shortLabel ? `results.grouping.short.${id}` : `results.grouping.${id}`,
    })
    return {
      id,
      label: intl.formatMessage({ id: "results.grouping.summary" }, { count, dimension }),
    }
  })
  return (
    <div className="grid min-w-0 gap-x-3 gap-y-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
      {dimensions.length > 0 && (
        <div className="min-w-0 overflow-x-auto px-0.5 pt-1 pb-1.5">
          <ToggleGroup
