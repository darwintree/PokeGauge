import { usageRules, usageData } from "./usage"
import { proxyPikalytics, proxySmogonStats } from "./usage-upstream"
export * from "./usage-upstream"
export { usageRules } from "./usage"
...
export default {
  async fetch(request, env, ctx): Promise<Response> {
    ...
    if (new URL(request.url).pathname.startsWith("/api/usage/")) return usageData(request, env, ctx)
